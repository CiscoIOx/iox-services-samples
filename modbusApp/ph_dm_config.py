...
def get_data_model( modelType, modelName):
    nbi_label = "nbi"
    ...
    con.request(
        "GET", 
        "/api/v1/mw/provisioning/" + modelType + "/" + modelName, 
        None, 
        headers
    )
...
def configure_data_model( modelType, fileName):
    nbi_label = "nbi"
    nbi_host = os.environ[nbi_label+"_IP_ADDRESS"]
    nbi_port = os.environ[nbi_label+"_TCP_9999_PORT"]
    ...
    con.request(
        "POST", 
        "/api/v1/mw/provisioning/" + modelType,
        payload,
        headers
    )
    response = con.getresponse()
...
def delete_data_model( modelType, modelName):
    nbi_label = "nbi"
    nbi_host = os.environ[nbi_label+"_IP_ADDRESS"]
    nbi_port = os.environ[nbi_label+"_TCP_9999_PORT"]

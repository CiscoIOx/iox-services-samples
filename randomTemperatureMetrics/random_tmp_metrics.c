...
#define TMP_RATE "TemperatureRate"
...
static void publish_temperature (void *arg)
{
	...
	mlib_metrics_meter_mark (s->s, TMP_RATE);
}
...
main(int argc, char **argv)
{
	...
	if (mlib_metrics_add_meter (s->s, TMP_RATE) != MLIB_SUCCESS) {
        mlib_log_err("%s: add meter failed", __FUNCTION__);
        return 1;
	}
